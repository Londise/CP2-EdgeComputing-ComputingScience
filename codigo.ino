...
int ldr = 8; //SENSOR (PINO ANALÓGICO 0)
int buzzer = 10; //BUZZER (PINO DIGITAL 10)
int ledVerde = 13; //LED VERDE (PINO DIGITAL 2)
int ledAmarelo = 12; //LED AMARELO (PINO DIGITAL 3)
int ledVermelho = 11;//LED VERMELHO (PINO DIGITAL 4)
...
void loop() {
  ...
  // ELSE IF DE LUMINOSIDADE
    ...
    else if (LuminosidadeLED >= 100 && LuminosidadeLED <= 800) {
      lcd.setCursor(0, 0);
  ...
  //POTÊNCIOMETRO INTERATIVO
  int sensorReading = analogRead(A1); // lê o potênciometro em A1
  int delayTime  = map(sensorReading, 0, 1023, 200, 1000); //MAPEIA OS VALORES PARA UMA ESCALA DE 200 A 1000
  ...
  vldr = analogRead(ldr);
  LuminosidadeLED = map(vldr, 8, 1015, 1015, 8);
  ...
  if (LuminosidadeLED > 800 || temperatura > 16.00 || temperatura < 10.00 || umidade > 70.00 || umidade < 50.00)
  {
    luzVermelho();
  }
  //Amarelo
  ...
  else if (LuminosidadeLED < 100 || (temperatura >= 11.00 && temperatura <= 15.00) || (umidade >= 50.00 && umidade <= 70.00))
  {
    luzVerde();
  }
...
void buzzerAlternativo() {
  ...
  if (currentMillis - previousBeepMillis >= 100) { // Intervalo de 1 segundo entre beeps
